...
class PostStoreController extends BaseController
{
    public function index(Request $request, $id)
    {
        ...
        if ($store != null) {
            ...
            }])->where('is_deleted', 0)->where('status', 'active')->where('store_id', $id)->where('postcategory_id', null)->get());
            ...
            $data = Page::with(['user' => function ($query) {
                $query->select('id', 'name');
            }])->whereIn('id', $postIds)->where('is_deleted', 0)->where('status', 'active')->where('store_id', $id)->orderBy('created_at', 'desc');
            ...
            $success['lastPosts'] = PageResource::collection(Page::with(['user' => function ($query) {
                $query->select('id', 'name');
            }])->where('is_deleted', 0)->where('status', 'active')->where('store_id', $id)->whereIn('id', $postIds)->orderBy('created_at', 'desc')->take(3)->get());
            // footer
