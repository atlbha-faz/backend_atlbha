...
class IndexStoreController extends BaseController
{
    public function index($id)
    {
        ...
        $seo = Seo::where('is_deleted', 0)->where('store_id',  $store->id)->select('id', 'google_analytics', 'snappixel', 'tiktokpixel', 'twitterpixel', 'instapixel', 'robot_link', 'key_words','metaDescription','header', 'footer','title')->first();
        if ($seo !== null) {
    ...
    public function storeProductCategory(Request $request)
    {
        ...
        if ($parent !== null) {
            ...
            $storeproducts = ProductResource::collection(Product::with(['store' => function ($query) {
                ...
                })->where('store_id', $store->id)->where('is_deleted', 0)->orderBy($s, $sort)->paginate($limit));
