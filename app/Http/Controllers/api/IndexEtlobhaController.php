...
class IndexEtlobhaController extends BaseController
{
    ...
    $query->select('id','name');}])->where('is_deleted', 0)->where('domain', 'atlbha')->where('special', 'special')->select('id','name','status','cover','special','store_id','created_at','category_id','subcategory_id','selling_price','stock')->get());
        } else {
