    ...
    public function index(Request $request)
    {
        ...
        if (!is_null(Section::where('id', 1)->where('is_deleted', 0)->where('status', 'active')->first())) {
            $success['products'] = AtlbhaIndexProductResource::collection(Product::with(['store' => function ($query) {
                ...
                $query->select('id', 'name');},'importproduct'])->where('is_deleted', 0)->where('admin_special', 'special')->select('id', 'name', 'status', 'cover', 'special', 'admin_special', 'store_id', 'created_at', 'category_id', 'subcategory_id', 'selling_price', 'stock','short_description')->get());
        } else {
