...
class SupplierController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $arrays = [[$request->civil_id, 1],[$request->bankAccountLetter, 21], [$request->website_image, 25], [$request->file, 20], [$request->national_address, 5]];
    ...
    public function update(Request $request)
    {
        ...
        $arrays = [[$request->civil_id, 1], [$request->file, 20], [$request->bankAccountLetter, 21], [$request->website_image, 25],[$request->national_address, 5]];
        foreach ($arrays as $file) {
            if (is_uploaded_file($file[0])) {
                $supplier = new FatoorahServices();
                $imageName = Str::random(10) . time() . '.' . $file[0]->getClientOriginalExtension();
