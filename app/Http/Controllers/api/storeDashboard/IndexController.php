    ...
    public function index()
      {
        ...
        $success['sales']=DB::table('order')->where('order_status','completed')->where('store_id',auth()->user()->store_id)->sum('total_price');
        ...
        for($i = 1; $i <= 12; $i++){ 
            ...
            $result= DB::table('order')->where('order_status','completed')->where('store_id',auth()->user()->store_id)->whereYear('created_at', date('Y'))->whereMonth('created_at', $i)->select(DB::raw('SUM(total_price - discount) as total'))->pluck('total')->first();
       $array_sales_monthly[date('M', mktime(0, 0, 0, $i, 10))]= $result!==null ? $result:0;
         ...
         for($i = 1; $i <= 12; $i++){ 
               $x = ($i-1)*7;
             $xx = ($i*7)-1;
           $result = DB::table('order')->where('order_status','completed')->where('store_id',auth()->user()->store_id)->whereDate('created_at', '>=',(date('Y-m-d' , strtotime("-".$xx." days"))))->whereDate('created_at','<=' ,(date('Y-m-d' , strtotime("-".$x." days"))))->select(DB::raw('SUM(total_price - discount) as total'))->pluck('total')->first();
              $array_sales_weekly[(date('Y-m-d', strtotime("-".$x." days"))).'/'.(date('Y-m-d', strtotime("-".$xx." days")))]= $result!==null ? $result:0;
       }
        
         for($i = 1; $i <= 12; $i++){ 
             $x = $i-1;
           $result= DB::table('order')->where('order_status','completed')->where('store_id',auth()->user()->store_id)->whereDate('created_at', date('Y-m-d' , strtotime("-".$x." days")))->select(DB::raw('SUM(total_price - discount) as total'))->pluck('total')->first();
