...
class IndexStoreController extends BaseController
{
    ...
    $query->select('id','name');}])->where('id',$order->id)->select('id','name','status','cover','special','store_id','created_at','category_id','subcategory_id','selling_price','stock')->first();
                    }
                }
...
'category'=> function ($query) {
    $query->select('id','name');}])->where('is_deleted', 0)->select('id','name','status','cover','special','created_at','category_id','subcategory_id','selling_price','stock')->where('status', 'active')
    ...
    public function productPage($domain, $id)
    {
        ...
        // } else {
            ...
            if ($store_id != null) {
                ...
                if ($import != null) {
                    $success['product'] = new importsResource(Product::join('importproducts', 'products.id', '=', 'importproducts.product_id')->where('products.is_deleted', 0)->where('importproducts.store_id', $store_id)->where('products.status', 'active')->where('products.id', $id)->first(['products.*', 'importproducts.price']));
    ...
    public function storeProductCategory(Request $request)
    {
        ...
        // } else {
           ...
           $query->select('id','domain','store_name');
            ...
            $query->select('id','name');}])->where('is_deleted', 0)->where('status', 'active')
