...
$resent_arrivede_by_category=Category::where('is_deleted',0)->where('store_id',$id)->whereHas('products', function ($query) use($id)  {
 ...
 ->where('store_id',$id)->whereDate('created_at', '>=', $oneWeekAgo)->where('category_id',$category->id)->get());
        ...
        $productsCategories=Product::where('store_id',$id)->groupBy('category_id')->whereHas('category', function ($query) {
  $query->where('is_deleted',0);
})->selectRaw('count(*) as total, category_id')->orderBy('total','DESC')->take(6)->get();
       foreach( $productsCategories as  $productsCategory){
        $success['PopularCategories'][]=new CommentResource(Category::where('is_deleted',0)->where('store_id',$id)->where('id', $productsCategory->category_id)->first());
       }
