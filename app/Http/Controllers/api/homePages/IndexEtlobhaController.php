...
class IndexEtlobhaController extends BaseController
{
    public function index(Request $request)
    {
        ...
        $success['store_activities'] = CategoryResource::collection(Category::with(['subcategory' => function ($query) {
            $query->select('id');
        }])->where('is_deleted', 0)->where('parent_id', null)->where('store_id', null)->whereIn('id', $categoriesStore)->where('status', 'active')->orderByDesc('created_at')->get());
        ...
        $success['atlbha_products'] = ProductResource::collection(Product::with(['store' => function ($query) {
            ...
            $query->select('id', 'name');
        }])->where('store_id', null)->where('is_deleted', 0)->where('for', 'etlobha')->whereNot('stock', 0)->orderByDesc('created_at')->select('id', 'name', 'cover', 'selling_price', 'purchasing_price', 'stock', 'less_qty', 'created_at', 'short_description', 'category_id', 'subcategory_id')->limit(20)
                ->get());
    ...
    public function services(Request $request)
    {
        if($request->has('all') && $request->input('all') !== null){
            $data = Service::where('is_deleted', 0)->whereNotIn('id', [75, 76])->where('status', 'active')->orderBy('created_at')->get();
