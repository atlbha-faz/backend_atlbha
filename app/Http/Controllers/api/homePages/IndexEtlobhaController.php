...
class IndexEtlobhaController extends BaseController
{
    public function index(Request $request)
    {
        ...
        $success['atlbha_products'] = ProductResource::collection(Product::with(['store' => function ($query) {
            ...
            $query->select('id', 'name');}])->where('store_id', null)->where('is_deleted', 0)->where('for', 'etlobha')->whereNot('stock', 0)->orderByDesc('created_at')->select('id', 'name', 'cover', 'selling_price', 'purchasing_price', 'stock', 'less_qty', 'created_at','short_description', 'category_id', 'subcategory_id') ->limit(20)
            ->get());
